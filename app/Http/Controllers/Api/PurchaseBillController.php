...
class PurchaseBillController extends ApiBaseController
{
    ...
    public function purchaseBillDetails( SalesCreateRequest $request)
	{
		DB::beginTransaction();
		try {
...
public function invoiceNumber(Request $request)
	{
		...
		$recentBill	= PurchaseBillDetail::select(["total_amount","party_customer_id",'invoice_number','tax_amount',DB::raw('DATE_FORMAT(order_date, "%d-%m-%Y") as invoiceDate')])->where('party_id',$request->party_id)
          ->get()->take(10);
