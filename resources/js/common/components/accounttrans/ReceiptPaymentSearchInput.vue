<template>
    ...
    >
        ...
        <a-select-option
            ...
            :key="newProduct.id"
            :value="newProduct.id"
            :label="newProduct.receiptpayment_mode"
            :product="newProduct"
            @click="handleOptionClick(newProduct)"
        >
            {{ newProduct.receiptpayment_mode }}
        </a-select-option>
    </a-select>
</template>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
                ...
                state.products1 = [
                    {
                        ...
                        receiptpayment_mode: propVal.productData.product.receiptpayment_mode,
                    },
                ];
        ...
        const fetchProducts = debounce((value) => {
            ...
            let filterString = `receiptpayment_mode lk "%${value}%"`;
            if (value == undefined) {
                filterString = "";
            } else if (
                typeof value === "object" ||
            ...
            let url = `receiptpayment_items?fields=id,xid,receiptpayment_mode&filters=${encodeURIComponent(
                filterString
            )}&limit=1000`;
        ...
        const handleOptionClick = (newProduct) => {
            if (newProduct.receiptpayment_mode === "F2 - Add new") {
                    ...
                    state.products = [
                        {
                            ...
                            receiptpayment_mode: props.productData.product.receiptpayment_mode,
                        },
                    ];
                }
            }
