...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const resetSearchInput = (propVal) => {
      console.log(propVal);
      console.log("aaaaaaaaaaaaaaaaaaaaaaaaa",propVal.productData)
      ...
      if (propVal.productData  && propVal.productData.product) {
        ...
        console.log("watch",state.searchTerm)
      } else {
        state.searchTerm = propVal.productData;
        //state.products = [];
      }
    ...
    const fetchProducts = debounce((value) => {
      ...
      axiosAdmin.get(url).then((response) => {
        state.products = response.data;
         console.log("addddd",state.searchTerm)
         
         
        if (state.searchTerm > 0) {
          console.log("addddd inside")
          state.products.forEach((data) => {
            if (data.id == state.searchTerm) {
              state.searchTerm = data.country_name;
            }
          });
        }
