<template>
    ...
    >
        ...
        <a-select-option
            ...
            :key="newProduct.id"
            :value="newProduct.id"
            :label="newProduct.station_name"
            :product="newProduct"
        >
            {{ newProduct.station_name }}
        </a-select-option>
    </a-select>
</template>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
                ...
                state.products = [
                    {
                        ...
                        station_name: propVal.productData.product.name,
                    },
                ];
        ...
        const fetchProducts = debounce((value) => {
            ...
            if (value != "") {
                state.fetching = true;
                const filterString = `station_name lk "%${value}%"`;
                let url = `stations?fields=id,xid,station_name&filters=${encodeURIComponent(
                    filterString
                )}&limit=10`;
