<template>
    ...
    >
        ...
        <a-select-option
            ...
            :key="newProduct.id"
            :value="newProduct.id"
            :label="newProduct.god_owns_name"
            :product="newProduct"
            @click="handleOptionClick(newProduct)"
        >
            {{ newProduct.god_owns_name }}
        </a-select-option>
    </a-select>
</template>
    ...
    setup(props, { emit }) {
                ...
                state.products = [
                    {
                        ...
                        god_owns_name:
                            propVal.productData.product.god_owns_name,
                    },
                ];
      ...
      if (event.key === "Delete" || event.key === "Backspace") {
          ...
          state.products = [
            {
              ...
              god_owns_name: props.productData.product.god_owns_name,
            },
          ];
        }
      }
        ...
        const fetchProducts = debounce((value) => {
            ...
            let filterString = `god_owns_name lk "%${value}%"`;
            if (value == undefined) {
                filterString = "";
            } else if (
                typeof value === "object" ||
            ...
            let url = `god-owns?fields=id,xid,god_owns_name&filters=${encodeURIComponent(
                filterString
            )}&limit=1000`;

            axiosAdmin.get(url).then((response) => {
                response.data.push({ id: null, god_owns_name: "" });
        ...
        const handleOptionClick = (newProduct) => {
            ...
            if (newProduct.god_owns_name === "F2 - Add new") {
