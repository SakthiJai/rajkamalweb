<template>
    ...
    >
        ...
        <a-select-option
            ...
            :key="newProduct.id"
            :value="newProduct.id"
            :label="newProduct.sales_type"
            :product="newProduct"
            @click="handleOptionClick(newProduct)"
        >
            {{ newProduct.sales_type }}
        </a-select-option>
    </a-select>
</template>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
                ...
                state.products = [
                    {
                        ...
                        sales_type: propVal.productData.product.sales_type,
                    },
                ];
        ...
        const fetchProducts = debounce((value) => {
            ...
            let filterString = `sales_type lk "%${value}%"`;
            if (value == undefined) {
                filterString = "";
            } else if (
                typeof value === "object" ||
            ...
            let url = `tax-category?fields=id,xid,sales_type&filters=${encodeURIComponent(
                filterString
            )}&limit=1000`;

            axiosAdmin.get(url).then((response) => {
                response.data.push({ id: null, sales_type: "" });
        ...
        const handleOptionClick = (newProduct) => {
            ...
            if (newProduct.sales_type === "F2 - Add new") {
          ...
          state.products = [
            {
              ...
              sales_type: props.productData.product.sales_type,
            },
          ];
        }
      }
