            ...
            <div class="table-responsive">
                ...
                >
                    <template #bodyCell="{ column, record }">
                        ...
                        <template v-if="column.dataIndex === 'inst_no'">
                            <a-typography-link
                        ...
                        <template v-if="column.dataIndex === 'action'">
                            <a-button
                            @click="editReturn(record.payment_voucher)"
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const setUrlData = (searchBy) => {
            ...
            datatableVariables.tableUrl.value = {
                url:
                    `${props.orderType}?fields=id,partyName{id,party_name,party_full_name}payment_voucher,inst_no,inst_type,station,amount,order_date&searchBy=` +
                    searchBy,
                filterString,
                filters: {
        ...
        const showDeleteConfirm = (id) => {
            ...
            const modal = Modal.confirm({
                ...
                onOk() {
                    axiosAdmin
                        .post(`payment/deletepayment/${id}`)
                        .then(() => {
                            setUrlData();
                            notification.success({
                                message: t("common.success"),
                                description: t(`Payment Deleted Successfully.`),
                            });
                        });
                    removeKeyListener();
                },
    ...
    methods: {
      ...
      this.$emit("child-select", voucher_number);
        ...
        editReturn(payment_voucher) {
      this.$emit("child-select", payment_voucher);
    },
    test(event) {
      switch (event.keyCode) {
        case 38: // Arrow up
