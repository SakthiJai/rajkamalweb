        ...
        <a-col :span="24">
            <div class="table-responsive">
              ...
              {{ formatDate(record.order_date) }}
                        ...
                        <template v-if="column.dataIndex === 'narration'">
                            {{ formatDate(record.narration) }}
                        </template>
                        ...
                        <template v-if="column.dataIndex === 'action'">
                            <a-button
                                @click="editReturn(record.voucher_number)"
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const setUrlData = (searchBy) => {
            ...
            datatableVariables.tableUrl.value = {
                url:
                    `${props.orderType}?fields=id,partyName{id,party_name,party_full_name}voucher_number,station,inst_type,inst_no,amount,order_date&searchBy=` +
                    searchBy,
                filterString,
                filters: {
        ...
        const showDeleteConfirm = (id) => {
            ...
            const modal = Modal.confirm({
                ...
                onOk() {
                    axiosAdmin.post(`receipt/deletereceipt/${id}`).then(() => {
                        setUrlData();
                        notification.success({
                            message: t("common.success"),
                            description: t(`Receipt Deleted Successfully.`),
                        });
                    });
                    removeKeyListener();
                },
    ...
    methods: {
          ...
          formatNumber(value) {
            ...
            return Number(value).toFixed(2); // Ensures two decimal places
        },
      ...
      this.$emit("child-select", voucher_number);
    },
