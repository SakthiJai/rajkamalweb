            ...
            <a-col :xs="24" :sm="24" :md="  12" :lg="12">
                        <a-form-item :label="$t('stock.party_name')" name="party_name" ref="input"
                            ...
                            <a-input v-model:value="formData.party_name" autocomplete="off"
                                ...
                                @focus="handleFocus($event)" @blur="handleBlur" @keydown="customerhandleKeydown($event)"  v-on:keyup.enter="validateLedger(); moveToNextField($event.target)"/>
                    ...
                    <a-col :xs="24" :sm="24" :md="12" :lg="12">
                        <a-form-item :label="$t('expense.expense_category')" name="expensecategory">
                        <CategorySearchInput @valueChanged="(productId) => (formData.expense_category = productId)
                        " @valueSuccess="getStockValue" :productData="data" v-on:keyup.enter="moveToNextField($event.target)"   id="form_item_expense_category" v-model="formData.expense_category" />
                        </a-form-item>
                ...
                <a-row :gutter="16">
                    <a-col :xs="24" :sm="24" :md="12" :lg="12">
                        <a-form-item
                            :label="$t('expense.expense_number')"
                            name="expense_number"   :help="rules.expense_number ? rules.expense_number.message : null"
                            :validateStatus="rules.expense_number ? 'error' : null" class="required">
                            <input
                            v-model="formData.expense_number"
                      id="form_item_expense_number"
                            class="ant-input css-dev-only-do-not-override-wosfq4"
                            />
                        </a-form-item>
                 </a-col>
                <a-col :xs="24" :sm="24" :md="12" :lg="12">
                   <a-form-item :label="$t('expense.orginal_invoice')" name="orginalinvoice" :help="rules.original_invoice_number ? rules.original_invoice_number.message : null"
                   :validateStatus="rules.original_invoice_number ? 'error' : null" class="required">
                           <input
                            v-model="formData.original_invoice_number"  id="form_item_original_invoice_number"
                            @keydown="validateMobile($event,index)"
                ...
                <a-row :gutter="16">
                    <a-col :xs="24" :sm="24" :md="12" :lg="12">
                        <a-form-item
                            :label="$t('expense.datepicker')"
                            name="bill_number" >
                        <input type="date" v-model="formData.order_date" class="ant-input css-dev-only-do-not-override-wosfq4" id="order_date" ref="DateInput" required

                         @keydown="handleKeydownDatePicker($event)"
                        @focus="changeColorOnFocus('order_date')" @blur="resetColorOnBlur('order_date')" />
                        </a-form-item>
                 </a-col>
                <a-col :xs="24" :sm="24" :md="12" :lg="12">
                   <a-form-item :label="$t('expense.payment_mode')" name="payment_mode"  id="form_item_payment_mode">
                            <a-select v-model:value="formData.payment_mode">
                      <a-select-option value="1" aria-selected="true">
                        UPI
                      </a-select-option>
                    </a-select>
                    </a-form-item>
                ...
                <a-row :gutter="16">
                    <a-col :xs="24" :sm="24" :md="12" :lg="12">
                        <a-form-item
                            :label="$t('expense.payment_mode_from')"
                            name="bill_number">
                             <a-select v-model:value="formData.payment_mode_from"   >
                      <a-select-option value="1" aria-selected="true">
                        Axis Bank
                      </a-select-option>
                    </a-select>
                ...
                <a-col :xs="24" :sm="24" :md="12" :lg="12">
                   <a-form-item :label="$t('expense.address')" name="address">
                   <input
                ...
                :successMessage="successMessage"
                @addEditSuccess="handleSuccess"
                @closed="handleClosePurchasePrints"
            />
                <!-- end purchase return -->

                <!-- bill return-->
                <BillReturnPrintModel
                v-if="isPurchaseBillModalVisible"
                :visible="isPurchaseBillModalVisible"
                :formData="formData"
                :url="url"
                :addEditType="addEditType"
                :pageTitle="pageTitle"
                        ...
                        <div class="dropdown">
                            ...
                            <ul v-if="isOpensave" class="dropdown-menus">
                              <li v-for="(item, index) in dropsitems" :key="index" class="dropdown-items" >
                                <span v-if="item.isSvg" v-html="item.icon" class="item-icon"></span>
...
import PrintModel from "../../stock-management/purchases/PrintModel.vue";
import BillReturnPrintModel from "../../stock-management/purchases/BillReturnPrintModel.vue";
import InvoiceItemSelectModel from "../../stock-management/purchases/InvoiceItemSelectModel.vue";
import CategorySearchInput from "../.../../../../../common/components/category/CategorySearchInput.vue";
import DeleteConfirmationPopupModel from "../../stock-management/purchases/DeleteConfirmationPopupModel.vue";
       ...
       let spinning=ref(false)
        ...
        const { salesItemColumns } = fields();
          formData=({
                ...
                items:[
                    {index:1,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:2,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:3,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:4,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:5,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:6,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:7,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:8,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:9,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                    {index:10,item_id:null,item_name:null,unit_id:null,quantity:'',free:'',mrp:null,single_unit_price:null,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:0,packing:null,selected:false},
                ],
        ...
        deleteItem()
{
      ...
      getInvoiceDetails(){
        ...
        if(this.selectedInvoice!="null")
        {
           // this.formData.bill_number=this.selectedInvoice;
            this.spinning = true;
            axiosAdmin
            .get("expense/getexpenseInvoiceDetails/"+this.selectedInvoice)
            .then(response => {  //console.log(response)
                ...
                this.formData.expense_number = response.data.invoiceData.expense_number;
                this.formData.original_invoice_number= response.data.invoiceData.original_invoice_number;
                this.formData.address= response.data.invoiceData.address;
                 this.formData.expense_category= response.data.invoiceData.expense_category;
                this.spinning= false;
        ...
        showProductModal(event,index) {
            ...
            if(event.keyCode==46 || event.keyCode==8)
                ...
                else if(event.keyCode==27)
                {
                ...
                else if(this.formData.party_name!="" && index>0 &&  event.key == 'Enter')
                 {
                    ...
                    else if(this.formData.items[index].item_id>0 && event.keyCode==13)
                    {
                        ...
                        this.handleItemProductFree(event, index);
                ...
                else if(this.formData.party_name!="" && index>0 &&  event.key == 'Tab')
                {
                    ...
                    this.selectedItermIndex =  index;
                    if(this.formData.expense_categoryss==undefined || this.formData.expense_categoryss=="" || this.formData.expense_categoryss.trim()==""){ ////console.log('+>',this.formData.bill_number)
                        document.getElementById("form_item_expense_categorys").focus();
                        event.preventDefault();
                        return false;
                    }
                    else if(this.formData.expense_number==undefined || this.formData.expense_number=="" || this.formData.expense_number.trim()==""){ ////console.log('+>',this.formData.bill_number)
                        document.getElementById("form_item_expense_number").focus();
                        event.preventDefault();
                        return false;
                    } else if(this.formData.original_invoice_number==undefined || this.formData.original_invoice_number=="" || this.formData.original_invoice_number.trim()==""){ ////console.log('+>',this.formData.bill_number)
                        document.getElementById("form_item_original_invoice_number").focus();
                        event.preventDefault();
                        return false;
                    }else if(this.formData.payment_mode==undefined || this.formData.payment_mode=="" || this.formData.payment_mode.trim()==""){ ////console.log('+>',this.formData.bill_number)
                        document.getElementById("form_item_payment_mode").focus();
                        event.preventDefault();
                        return false;
                    }
                ...
                else if(this.formData.party_name!="" && index>0 && (event.keyCode !== 9 && event.keyCode!=121 && event.key!="F8")  )
            ...
            if(this.formData.party_name!=""  &&  event.key == 'Enter')
                {
                    ...
                    if(this.formData.bill_number==undefined || this.formData.bill_number=="" || this.formData.bill_number.trim()==""){ ////console.log('+>',this.formData.bill_number)
                       // document.getElementById("form_item_expense_category").focus();
                        event.preventDefault();
                        return false;
                    }
   ...
   handleClose() {
        ...
        checkMaxFree(index,event)
        {
            //console.log(this.formData.items[index].maxquantity,event.target.value,event.key)
            // if(Number(this.formData.items[index].free)>Number(this.formData.items[index].maxquantity))
            // {

            //     notification.warning({
            //             placement:  "bottomRight",
            //             message: "Stock free Going to be Negative!",
    ...
    if (this.validateLedger()) {
        console.log('submit', event);
            this.spinning = true;
            axiosAdmin
                .post("expense/expenseDetail", this.formData)
                .then(response => {
                    this.spinning = false;
    ...
    if (!this.formData.party_name || typeof this.formData.party_name !== 'string' || this.formData.party_name.trim() === "") {
    ...
    if (!this.formData.expense_number || typeof this.formData.expense_number !== 'string' || this.formData.expense_number.trim() === "") {
        this.rules.expense_number = {
            message: 'Expense Number is required.',
        };
        isValid = false;
    } else {
        this.rules.expense_number = null;
    }



    if (!this.formData.original_invoice_number || typeof this.formData.original_invoice_number !== 'string' || this.formData.original_invoice_number.trim() === "") {
        this.rules.original_invoice_number = {
            message: 'Original Invoice number is required.',
        };
        isValid = false;
    } else {
        this.rules.original_invoice_number = null;
    }
    ...
    return isValid;
        ...
        checkFree(index,event)
        {
            //console.log(index,'price focus',this.formData.items[index].free);
            const free = this.formData.items[this.selectedItermIndex].free;
            if(free==undefined || free=="" || free.trim()=="")
            {
                document.getElementById("item_product_free_"+index).focus();
                return false;
            }
        ...
        addressEvent(event)
        {
            if((event.keyCode==121 || event.keyCode==13)&& (this.formData.address!=undefined  && this.formData.address!=null && this.formData.address!="" ))
            {

                this.saveSalesEntry();
            }
            else if((event.keyCode==121 || event.keyCode==13)&& this.formData.address==undefined )
            {
                ...
                else if(this.formData.party_customer_id==0 || this.formData.party_customer_id=="")
                {
        ...
        {
            let that = this;
            console.log(this.recentEventId.target.id)
            Modal.confirm({
            title: 'Confirmation',
        ...
        gotoFree(index,event)
        {
            console.log(event.keyCode,Number(this.formData.items[index].free))
            if(event.keyCode==13 && Number(this.formData.items[index].free)>0)
            {
                console.log("Enter key pressed")
