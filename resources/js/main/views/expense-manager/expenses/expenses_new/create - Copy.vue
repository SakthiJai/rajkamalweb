      ...
      <admin-page-table-content>
        <a-card class="page-content-container mt-5 mb-5">
          <a-form layout="vertical">
            <a-row :gutter="16">
              <a-col :xs="24" :sm="24" :md="taxTypeLayoutValue" :lg="taxTypeLayoutValue">
                <a-form-item
              ...
              <a-col v-show="isPartyDetailsVisible" :xs="24" :sm="24" :md="6" :lg="6">
                <a-form-item
                ...
                >
                  <a-input
                    ...
                    "
                    @keydown="customerhandleKeydown($event, 'party_name')"
                    @focus="customerhandleKeydown($event)"
                    :tabindex="formData.tax_type === 'with_gst' ? 2 : -1"
                  ...
                  <a-input
                    ...
                    id="party_id"
                    :placeholder="
                      $t('common.placeholder_default_text', [$t('stock.party_')])
                    "
                  />
                </a-form-item>
              ...
              <a-col :xs="24" :sm="24" :md="12" :lg="12">
                <a-form-item
                  ...
                  class="required"
                  :help="
                    rules.expense_category_id ? rules.expense_category_id.message : null
                  "
            ...
            <a-row :gutter="16">
              ...
              <a-col :xs="24" :sm="24" :md="12" :lg="12">
                <a-form-item
                  ...
                  name="expense_date"
                  :help="rules.expense_date ? rules.expense_date.message : null"
                  :validateStatus="rules.expense_date ? 'error' : null"
                >
                  <input
                    type="date"
            ...
            <a-row :gutter="16">
              <a-col :xs="24" :sm="24" :md="12" :lg="12">
                <a-form-item
                  ...
                  name="orginalinvoice"
                  :help="
                    rules.orginal_bill_number ? rules.orginal_bill_number.message : null
                  "
                  :validateStatus="rules.orginal_bill_number ? 'error' : null"
                  class="required"
                >
                  <input
                    ...
                    @input="formData.orginal_bill_number = $event.target.value"
                    @keydown="focusNext($event, 'orginal_bill_number')"
              ...
              <a-col :xs="24" :sm="24" :md="12" :lg="12">
                <a-form-item
                  ...
                  name="bill_amount"
                  :help="rules.bill_amount ? rules.bill_amount.message : null"
                  :validateStatus="rules.bill_amount ? 'error' : null"
                  class="required"
                >
                  <input
                    autocomplete="off"
            ...
            <a-row :gutter="16">
              <a-col :xs="24" :sm="24" :md="12" :lg="12">
                <a-form-item
                  ...
                  name="orginalinvoice"
                  :help="rules.tax_amount ? rules.tax_amount.message : null"
                  :validateStatus="rules.tax_amount ? 'error' : null"
                >
                  <input
                    autocomplete="off"
              ...
              <a-col :xs="24" :sm="24" :md="12" :lg="12">
                ...
                >
                  <select
                    ...
                    :allowClear="true"
                    optionFilterProp="label"
                    @keydown="focusNext($event, 'payment_mode_id')"
            ...
            <a-row :gutter="16">
              <a-col :xs="24" :sm="24" :md="12" :lg="12">
                <a-form-item :label="$t('expense1.note')" name="notes">
                  <textarea
                    ...
                    class="ant-input css-dev-only-do-not-override-wosfq4"
                    :tabindex="formData.tax_type === 'with_gst' ? 10 : 9"
                  />
                </a-form-item>
              </a-col>
              <a-col :xs="24" :sm="24" :md="3" :lg="3">
                <a-form-item :label="$t('expense1.upload')" name="image" id="img">
                  <UploadExpenses
                    ref="uploadComponent"
                    :formData="formData"
                    folder="expenses"
                    ...
                    "
                    :tabindex="formData.tax_type === 'with_gst' ? 11 : 10"
                  />
            ...
            <a-row :gutter="16" class="mt-20 mb-20">
              ...
              <a-col :xs="24" :sm="24" :md="3" :lg="3">
                <a-button
                  class="savecolse"
                  @click="onclose"
                  :tabindex="formData.tax_type === 'with_gst' ? 13 : 12"
                  block
                >
                  <span class="shortcut">
                    <code>Esc</code>
...
export default {
  ...
  data() {
    return {
      ...
      expenseCategories: [],
      PaymentModes: [],
      selectedExpense: null, // Holds the selected expense ID
  ...
  methods: {
    ...
    validateField(fieldName) {
      ...
      switch (fieldName) {
        case "party_name":
          if (!this.formData.party_name || this.formData.party_name.trim() === "") {
        ...
        case "tax_type":
          if (this.formData.tax_type) {
            if (!this.formData.tax_type || this.formData.tax_type.trim() === "") {
              this.rules.tax_type = {
                message: "This Field is required.",
              };
              isValid = false;
      ...
      this.$nextTick(() => {
        ...
        if (event.keyCode === 39) {
          ...
          if (currentColIndex < rows[currentRowIndex].length - 1) {
            const nextField = rows[currentRowIndex][currentColIndex + 1];
            this.$nextTick(() => this.$refs[nextField].focus());
          }
        }
        ...
        // Handle left arrow key (move to previous field in the same row)
          ...
          if (currentColIndex > 0) {
            const previousField = rows[currentRowIndex][currentColIndex - 1];
            this.$nextTick(() => this.$refs[previousField].focus());
          }
        }
      });
        ...
        axiosAdmin
          ...
          .then((response) => {
            ...
            if (response.data.expenseData.party_id != "") {
              this.formData.party_id = response.data.expenseData.party_id;
              this.formData.party_name = response.data.partyData.party_name;
            }

            this.formData.image = response.data.expenseData.image;
            this.formData.image_url = response.data.expenseData.image_url;
            this.formData.orginal_bill_number =
              response.data.expenseData.orginal_bill_number;
            this.formData.bill_amount = response.data.expenseData.bill_amount;
            this.formData.tax_amount = response.data.expenseData.tax_amount;
            this.formData.payment_mode_id = response.data.expenseData.payment_mode_id;
            this.formData.notes = response.data.expenseData.notes;
            this.formData.order_date = response.data.expenseData.order_date;
            this.formData.expense_date = response.data.expenseData.expense_date;
            this.formData.tax_type = response.data.expenseData.tax_type;
            this.isPartyDetailsVisible = this.formData.tax_type === "with_gst";
            if (this.formData.tax_type === "with_gst") {
              this.taxTypeLayoutValue = 6;
            } else {
              this.taxTypeLayoutValue = 12;
    ...
    onclose() {
      ...
      const modal = Modal.confirm({
        ...
        onOk() {
          localStorage.setItem("selectedExpense", null);
          window.removeEventListener("keydown", that.handleKeyPress, true);
          that.$router.push({
            name: "admin.expenses1.index",
          });
        },
