        ...
        <template #header>
            <a-row :gutter="16">
                <a-col :xs="24" :sm="24" :md="4" :lg="4">
                    <h3 class="gstinvoice">Expenses Entry List</h3>
                </a-col>
                ...
                <a-col :xs="24" :sm="24" :md="7" :lg="7" style="text-align:right;">
                    ...
                    ">
                            <a-button type="primary" class="creating" @click="createNewExpenseEntry()">
                                <PlusOutlined />
                                Create F2
                            </a-button>
                    </template>
...
import ExpenseTable from "./expensesTable.vue";
...
import { createVNode ,onBeforeUnmount} from 'vue';
...
export default {
    ...
    data() {
        return {
            ...
            isLoading: false,
            isF2Navigation: false,
            fullPage: true
        };
    },

    mounted() {
    document.getElementById('expanseList').addEventListener('keydown', this.expansehandleKeydown);
    this.autoFocusInput();
    ...
    methods:
    {
        selectDateRange(range) {
            ...
            this.expensesTableRef.setUrlData(this.selectedRange);
    ...
    this.createNewExpenseEntry(); 
        ...
        beforeUnmount() {
    document.getElementById('expanseList')
        ?.removeEventListener('keydown', this.expansehandleKeydown);
},
...
updateselect(value) {
    ...
    console.log("isf2navigation=>",this.isF2Navigation);    
    if (this.isF2Navigation) {
        this.isF2Navigation = false;
        return;
    }

    this.isDropdownOpen = false;
    localStorage.setItem("selectedExpense", value);
    ...
    if (event.key === "Escape" || event.keyCode === 27) {
        console.log("ESC key press disabled");
        return; 
    }
    
    console.log("Key code:", event.keyCode, this.expensesTableRef);
    this.expensesTableRef.test(event);
},
    ...
    setup() {
      ...
      // Put your cleanup code here (e.g., remove event listeners, clear intervals)
    });
        const {
