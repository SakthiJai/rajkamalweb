  ...
  methods: {
    ...
    checkproductDuplicate(event) {
      ...
      if (
        ...
        axiosAdmin
          .get("parentDuplicate/" + this.formDataLedger.party_name + "/0")
          .then((response) => {
            console.log(response);
            // Toastr Notificaiton
            if (response == 500) {
    ...
    validateStateField() {
      ...
      // Only validate if a value has been selected
      if (stateValue === null || stateValue === undefined) {
        // Don't show validation error until a value is attempted
        ...
        // Clear validation when a valid value is selected
        this.rules.stock_state = null;
      }
    ...
    handleKeydown(event) {
      ...
      if (
        ...
        return true;
      } else if (event.key === "Tab" && event.target.id === "gstnumber") {
        event.preventDefault();
        document.getElementById("pannumber")?.focus();
      } else if (event.key === "Tab" && event.target.id === "pannumber") {
        event.preventDefault();
        document.getElementById("gstnumber")?.focus();
      } else if (
        (event.key === "Escape" || event.keyCode === 27) &&
        !["country_search", "state_search", "country_search", "parent_ledger"].includes(
          event.target.id
        )
      ) {
        this.closePay();
        ...
        if (targetId === "gstnumber" || targetId === "pannumber") {
          this.fieldFocus(event);
        }
      } else if (event.key === "ArrowRight") {
        this.formDataLedger.gst_available = this.formDataLedger.gst_available = 1;

        const gstRadioElement = document.getElementById("gstradio");
        if (this.formDataLedger.gst_available === 1) {
          gstRadioElement.style.display = "none";
          console.log("Moved to Without GST");
        } else {
          gstRadioElement.style.display = "block";
    ...
    fieldFocus(event) {
      ...
      if (event.target.id == "gstnumber" && (!gstNumber || gstNumber === "")) {
        this.$refs.paninput.focus(); // Focus on PAN input if GST is empty
      } else if (
        event.target.id == "pannumber" &&
        (!gstNumber || gstNumber === "") &&
        (!panNumber || panNumber === "")
      ) {
        this.$refs.gstinput.focus(); // Focus on GST input if both are empty
        this.rules.with_gst_number = {
          message: "Please enter either GST Number or PAN Number",
        };
      } else {
        // Validation on user input
        if (event.target.id === "gstnumber" && gstNumber && !isGstValid) {
          this.rules.with_gst_number = {
            message: "Please Enter valid GST Number",
          };
        } else if (event.target.id === "pannumber" && panNumber && !isPanValid) {
        ...
        if (!this.notificationShown) {
          ...
          axiosAdmin.get("/getGstDetails/GSPTN1351G").then((response) => {
            if (response != null && response != "") {
              if (response.data != "") {
                this.formDataLedger.ledger_type = "Registered";
