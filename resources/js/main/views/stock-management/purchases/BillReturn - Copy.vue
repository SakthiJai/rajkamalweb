                                    ...
                                    <tbody id="itemtable">
                                        <tr  v-for="(prod, index) in formData.items" :key="prod.unique" :id="prod.unique">
                                            ...
                                            <td style="width:6%">
                                                ...
                                                @blur="updateAggg(index),checkMaxQuantity(index,$event)"
...
export default {
    ...
    mounted() {
        ...
        // this.formData.items[index].discount_type_id = "3";


        document.getElementById('salesbillvue').addEventListener('keydown', this.handleKeyDowning);
        this.autoFocusInput();
      ...
      getInvoiceDetails(){
        ...
        if(this.selectedInvoice!="null")
        {
            ...
            .then(response => {  console.log(response)
                ...
                console.log("aaaaaa",this.formData);
                console.log("aaaaa",this.formData.order_date);
                this.formData.invoice_date                =   response.data.invoiceData.invoice_date;
...
console.log("checking the loggggg")
                this.spinning = false;
                ...
                if(response.data.invoiceItems && response.data.invoiceItems.length>0)
                {
                    ...
                    response.data.invoiceItems.forEach((data,index)=>{
                        ...
                        this.formData.items[index].freeQty                  = data.free;
...
hasValidInput(target) {
    ...
    this.formData.items[this.selectedItermIndex].discount_type_id = 'Lumpsum';
    ...
    //this.$emit("closedbyclose");
        ...
        if (index > 0) {
            document.getElementById("item_product_disc_" + (index - 1)).focus();
        } else {
            document.getElementById("form_item_bill_number").focus(); // Or another logical previous field
        }
        event.preventDefault();
        return false;
    }
          ...
          onclose()
            ...
            if(this.selectedItermIndex!=undefined){
                ...
                if(prodName==undefined || prodName=="" || prodName.trim()=="")
                {
                    ...
                    setTimeout(function(){
                        document.getElementById("item_product_price_"+cindex).focus();
                    },500)
        ...
        this.isProuctsModalVisible  =false;
        document.getElementById("item_product_price_0").focus();
        //this.focusProductList();
        },
        focusProductList()
        {
        ...
        getTotalAmount(type,index)
        {
            ...
            for (var i = 0; i < this.formData.items.length; i++)
            {
                ...
                if (element.item_id > 0 && element.discount_rate >= 0) {
                    ...
                    if (element.discount_type_id == 'Lumpsum') {
     ...
     handleKeydownDatePickers(event) {
    if (event.key === 'Enter' || event.key === 'Tab') {
        event.preventDefault();

         if (!this.formData.invoice_date) {
            this.formData.invoice_date = new Date().toISOString().split('T')[0];
        } else {
            const currentDate = new Date(this.formData.invoice_date);
            this.formData.invoice_date = currentDate.toISOString().split('T')[0];
        }

        document.getElementById("invoice_date").value = this.formData.invoice_date;

         setTimeout(() => {
            this.foucsproducting();
        }, 200);
    }
},
        foucsproducting() {
...
let rate = document.getElementById(`item_product_price_${index}`).value;
console.log("dddddddddddddddd", rate);
...
if (isNaN(rate)) {
    ...
    let totalRate = Number(document.getElementById("total_rate_details").value) || 0;

    totalRate += rate;


    document.getElementById("total_rate_details").value = totalRate;

    console.log("Updated total rate:", totalRate);
}
        ...
        updateAgg()
        {
            ...
            this.formData.items.forEach((elem,index)=>{
                ...
                // if(elem.rate!=undefined && elem.rate!=null && Number(elem.rate)>0)
                // {
                //     //count =count+1;
                //     rate = rate+Number(elem.rate);
                // }
          ...
          //  document.getElementById("total_rate_details").value = rate.toFixed(0);
        ...
        gotoNext(index,event)
        {
            ...
            if (event.shiftKey && event.key === "Tab") {
        if (event.target.id == 'item_product_freeQty_' + index) {
            document.getElementById('item_product_quantity_' + index).focus();
            event.preventDefault();
            return false;
        } else if (event.target.id == 'item_product_quantity_' + index) {
            document.getElementById('item_product_price_' + index).focus();
            event.preventDefault();
            return false;
        }
        ...
        addressEvent(event) {
        if ((event.keyCode === 119 || event.keyCode === 13) && (this.formData.bill_number !== undefined && this.formData.bill_number !== null && this.formData.bill_number !== "")) {
            this.$nextTick(() => {

                this.$refs.DateInputt.focus();
            });
        } else if (event.keyCode === 27) {
...
resetFormData() {
     document.getElementById("total_quantity_details").value = 0;
     document.getElementById("total_free_quantity_details").value = 0;
     document.getElementById("total_remQty_quantity_details").value = 0;
     document.getElementById("total_with_disc_details").value = 0;
     document.getElementById("total_goods_value").value = 0;
      document.getElementById("igst_amount_2").value = 0;
    ...
    document.getElementById("total_quantity_details").value = TotalQty.toFixed(2);


    // Reset the value of the total_prod_count input field  total_quantity_details  grand_total
    const totalQualityInput1 = document.getElementById("total_quantity_details");
if (totalQualityInput1) {
    totalQualityInput1.value = (0).toFixed(2);
}
    ...
    const totalRateInput = document.getElementById("total_rate_details");
    if (totalRateInput) {
        totalRateInput.value = (0).toFixed(2);
    }
