        ...
        <a-card class="page-content-container mt-5 mb-5">
            ...
            <a-col :xs="24" :sm="24" :md="8" :lg="8">
                ...
                <a-row :gutter="16">
                    ...
                    <a-col :xs="24" :sm="24" :md="8" :lg="8">
                        <a-form-item :label="$t(`Shipping Address`)" name="address" :help="rules.address ? rules.address.message : null
                            " :validateStatus="rules.address ? 'error' : null" >
                            <input autocomplete="off" id="form_item_address" class="ant-input css-dev-only-do-not-override-wosfq4" v-model="formData.shipping_address"  @keydown="showAddressModal($event)"  placeholder="Press Spacebar" />

                            <a-input  hidden v-model:value="formData.party_shippingaddress_id" id="party_shippingaddress_id"
                        :placeholder="$t('common.placeholder_default_text', [$t('stock.party_')])" @blur="" />
      ...
      getInvoiceDetails(){
        ...
        if(this.selectedInvoice!="null")
        {
            ...
            .then(response => {  console.log(response)
                ...
                this.formData.party_state               =   response.data.partyDetails.stock_state,
                ...
                if(response.data.invoiceItems && response.data.invoiceItems.length>0)
                {
                    ...
                    response.data.invoiceItems.forEach((data,index)=>{
                        ...
                        total_price                                         = total_price+Number(data.single_unit_price);
                        ...
                        document.getElementById('total_rate_details').value = total_price;
                        ...
                        //Disc Amount
                         document.getElementById('item_product_disc_value_'+index).value=document.getElementById('item_product_disc_'+index).value;
                        finalIndex = index;
                        ...
                        totalTaxSum += Number(totalTaxValue);

                    })
                    console.log("checking aaaaaaaaaaaaa", totalTaxSum)
                        document.getElementById('total_discount_text1').innerHTML = totalTaxSum.toFixed(2);
                   ...
                   console.log("cess =>",this.formatCurrency(cessAmount))
                    ...
                    document.getElementById('igst_amount_3').value = this.formatCurrency(cessAmount);
                     ...
                     //Calculate the Igst igst_amount_4 grand_total discount_rate
                    var calculateIgstAll = document.getElementById("igst_amount_2").value;
                    var calculatecessAll = document.getElementById("igst_amount_3").value;
                    var calculateCgstAll = document.getElementById("igst_amount_0").value;
                    var calculateSgstAll = document.getElementById("igst_amount_1").value;

                    console.log('Number(calculateIgstAll)',calculateIgstAll)
                    var calculatedAll = 0;



                        calculatedAll = Number(calculateIgstAll) + Number(calculatecessAll)
                    console.log("rrrrr",document.getElementById("igst_amount_2").value)
                        document.getElementById("igst_amount_4").value = calculatedAll.toFixed(2);
                    ...
                    document.getElementById('grand_total').innerHTML = this.formatCurrency((grand_total + (calculatedAll)));
    ...
    //this.$emit("closedbyclose");
        ...
        getQuantity(index, event) {
          ...
          //  }
            ...
            const additionalDetails = Number(document.getElementById("igst_amount_2").value.replace(/,/g, '')) || 0;
            const taxableAmount = Number(this.totalAmount) || 0;
            const invoiceValue = additionalDetails + taxableAmount;
            document.getElementById("grand_total").innerHTML = this.formatCurrency(invoiceValue);
            this.formData.total = invoiceValue;
