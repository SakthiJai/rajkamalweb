                                    ...
                                    <tbody id="itemtable">
                                        ...
                                        <tr v-for="(header, index) in formData.invoiceitems" :key="index" class="invoicevalues">
                                            ...
                                            <td style="width:8%;text-align:right">
                                               ...
                                               <input id="total_without_disc_details"
...
export default {
    ...
    data() {
        ...
        isSvg: true  }
            ...
            headers: ['Product', 'Packing',  'Rate','Qty','Free','Billing Qty','Value Before Discount','Disc Type', 'Disc Value', 'Value After Discount','Tax %','₹ Taxable Amount'],
    ...
    //this.$emit("closedbyclose");
        ...
        getTotalAmount(type,index)
        {
            ...
            for (var i = 0; i < this.formData.items.length; i++)
            {
                ...
                if(element.item_id>0 && element.single_unit_price>0 && element.quantity>0 )
                {
                    ...
                    total = total + Number(element.single_unit_price) * Number(tempQuantity);
        ...
        getQuantity(index, event) {
            ...
            if (totalDiscount > 0) {
                ...
                console.log("withoutDisc->",withoutDisc,withoutDisc.replace(/,/g,''),totalDiscountByItem);
            ...
            document.getElementById("grand_total").innerHTML     = totalAmount>0? this.formatCurrency((totalAmount+totalSgstAmount+totalCgstAmount+cessAmount)-Number(totalDiscount)):0;
            this.formData.total                                   = (totalAmount+totalSgstAmount+totalCgstAmount+cessAmount)-Number(totalDiscount);
            //document.getElementById('total_discount_text1').innerHTML = this.formatCurrency(totalDiscount)
        ...
        updateAgg()
        {
            ...
            this.formData.items.forEach((elem)=>{
                ...
                if(elem.withDisc!=undefined && elem.withDisc!=null && Number(elem.withDisc)>0)
                {
                    //count =count+1;
                    total_with_disc_details = total_with_disc_details+Number(elem.withDisc);
                }
           ...
           // document.getElementById("total_prod_count").value="Total : "+count+" Product";
            ...
            document.getElementById("total_free_quantity_details").value = freequantity.toFixed(2);
            document.getElementById("total_remQty_quantity_details").value = remQty.toFixed(2);
            document.getElementById("total_without_disc_details").value = total_without_disc_details.toFixed(2);
