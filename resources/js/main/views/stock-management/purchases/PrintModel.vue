  ...
  >
        ...
        <fieldset>
          ...
          <a-row :gutter="16">
            ...
            >
              ...
              <!-- <option value="2">E-Invoice Billing</option> -->
            </select>
    ...
    <template #footer>
      ...
      ref="printButton"
        ...
        id="btn-Ledger"
        title="Ledger"
         @click="setFocusedButton('print'); printpdf()"
        @keydown="handleKeydowns"
        tabindex="0"
        class="btn default-btn ng-star-inserted"
      >
        <span class="box" @click="printpdf"
          ><span class="shortcut ng-star-inserted"><code>P</code></span
      ...
      ref="einvoiceButton"
        ...
        id="btn-Ledger"
        title="Ledger"
         @click="setFocusedButton('einvoice'); einvoice()"
         tabindex="0"
        class="btn default-btn ng-star-inserted"

      >
        <span class="box">
          <span class="ng-star-inserted">E-Invoice</span>
          <svg
            focusable="false"
            data-icon="whatsapp"
            width="1em"
        ...
        <!-- svg and path place replace here fix the svg    <E _ Invoice>-->
        <span class="effect"></span>
      </button>
      ...
      <button
        ...
        id="btn-Ledger"
        title="Ledger"
        @click="setFocusedButton('ewaybill'); ewaybill()"
        tabindex="0"
                class="btn default-btn ng-star-inserted"
      >
        <span class="box">
          <span class="ng-star-inserted">E-Way</span>
          <svg
            focusable="false"
            data-icon="whatsapp"
            width="1em"
        ...
        <!-- svg and path place replace here fix the svg    <E _ Way billl>-->
        <span class="effect"></span>
      </button>
      ...
      <button
        ...
        title="Ledger"
        tabindex="0"
        @click="setFocusedButton('preview'); previewpdf()"
        class="btn default-btn ng-star-inserted"
      >
        <span class="box"
          ><span class="shortcut ng-star-inserted"><code>F</code></span
          ><span class="ng-star-inserted">Preview</span>
        </span>
        <span class="effect"></span>
      </button>
      ...
      <button
        ...
        title="Ledger"
        tabindex="0"
         @click="setFocusedButton('close'); closePrint()"
        class="btn default-btn ng-star-inserted"
      >
        <span class="box"
...
export default defineComponent({
  ...
  created() {
    console.log("created:", this.billsItems);
  },

  ewaycreated()
  {
    console.log("done wrokign:", this.EwaybillsItems);
  },
  components: {
    PlusOutlined,
    LoadingOutlined,
  ...
  data() {
    return {
      ...
      isEInvoiceModalVisible:false,
      isEwayBillModelVisible:false,
      spinning: false,
      formData: {
        business_type: "1",
      },
  ...
  mounted() {
    ...
    console.log("Mounted: Adding keydown listener");
    document.addEventListener("keydown", this.handleKeyDownPrint);
    this.autoFocusInput();
    this.$nextTick(() => {
      console.log("Mounted: Focusing Print button");
      if (this.$refs.printButton) {
        this.$refs.printButton.focus();
        console.log("Print button focused");
      } else {
        console.log("Print button ref not found");
      }
    });
  },

  beforeDestroy() {
    console.log("BeforeUnmount: Removing keydown listener");
    document.removeEventListener("keydown", this.handleKeyDownPrint);
  },
...
setFocusedButton(button) {
      ...
      console.log("Focused button set to:", button);
    },

    handleKeyDownPrint(event) {
  ...
  //   if (this.formData.business_type === "2") {
  //     this.loading = true;
  //   } else {
  //     this.loading = false;
  //   }
  ...
  einvoice() {
    this.handleEInvoiceSelection();
  },


  handleEInvoiceSelection() {
    //this.spinning=false,
    this.isEInvoiceModalVisible=true;
    console.log("E-Invoice Billing selected, perform action heres...");
  },

  ewaybill()
  {
    this.handleEWaySelection();
  },
  handleEWaySelection() {
    //this.spinning=false,
    this.isEwayBillModelVisible=true;
    console.log("E-Way Billing selected, perform action heres...");
  },
    ...
    if (event.key === "Escape") {
      ...
      if (this.focusedButton === "print") {
        this.printpdf();
      } else if (this.focusedButton === "einvoice") {
        this.einvoice();
      } else if (this.focusedButton === "ewaybill") {
        this.ewaybill();
      }
    }
  },
  ...
  'formData.business_type'(newValue) {
    if (newValue === "2") {
      this.spinning = true;  // Start loading
      setTimeout(() => {
        this.handleEInvoiceSelection();
        this.spinning = false;
      }, 1200);
