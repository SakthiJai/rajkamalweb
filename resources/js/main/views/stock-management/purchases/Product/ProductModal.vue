        ...
        <a-col :xs="24" :sm="24" :md="6" :lg="6">
          <a-form-item name="sales_names">
            <a-select v-model:value="formData.sales_names">
              <a-select-option key="1" value="1"> Description </a-select-option>
              <a-select-option key="2" value="2"> Any Where </a-select-option>
              <a-select-option key="3" value="3"> Top Sales </a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
      </a-row>
    ...
    <a-row :gutter="16">
      <a-col :xs="24" :sm="24" :md="6" :lg="6">
        <fieldset style="height: 167px">
          <legend class="font-style-in-prouct-model">Purchase Info</legend>
          <a-row :gutter="16">
          ...
          <a-row :gutter="16">
            <a-col :xs="10" :sm="10" :md="10" :lg="10"> Pur. Rate </a-col>
            <a-col :xs="4" :sm="4" :md="4" :lg="4">
              <span>: ₹</span>
            </a-col>
            <a-col :xs="10" :sm="10" :md="10" :lg="10">
          ...
          <a-row :gutter="16">
            <a-col :xs="10" :sm="10" :md="10" :lg="10"> Cost+Tax </a-col>
            <a-col :xs="4" :sm="4" :md="4" :lg="4">
              <span>: ₹</span>
            </a-col>
            <a-col :xs="10" :sm="10" :md="10" :lg="10">
          ...
          <a-row :gutter="16">
            <a-col :xs="10" :sm="10" :md="10" :lg="10"> Pur. Disc </a-col>
            <a-col :xs="4" :sm="4" :md="4" :lg="4">
              <span>: ₹</span>
            </a-col>
            <a-col :xs="10" :sm="10" :md="10" :lg="10">
      ...
      <a-col :xs="24" :sm="24" :md="6" :lg="6">
        <fieldset style="height: 167px">
          ...
          <a-row :gutter="16">
            <a-col :xs="10" :sm="10" :md="10" :lg="10"> Deal Free </a-col>
            <a-col :xs="4" :sm="4" :md="4" :lg="4">
              <span>: ₹</span>
            </a-col>
            <a-col :xs="10" :sm="10" :md="10" :lg="10">
          ...
          <a-row :gutter="16">
            <a-col :xs="10" :sm="10" :md="10" :lg="10"> W/o Deal </a-col>
            <a-col :xs="4" :sm="4" :md="4" :lg="4">
              <span>: ₹</span>
            </a-col>
            <a-col :xs="10" :sm="10" :md="10" :lg="10">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const onClose = () => {
      console.log("Product Closed call");
    ...
    const onSelectChange = (changableRowKeys) => {
      ...
      setTimeout(function () {
        ...
        myElem.forEach(function (elem, index) {
          try {
            const tr = elem.closest ? elem.closest("tr") : null;
            if (!tr) return;
            name = tr.getElementsByTagName("td")[1].innerHTML.replace(/<[^>]*>?/gm, "");
            packing = tr.getElementsByTagName("td")[2].innerHTML.replace(/<[^>]*>?/gm, "");
            quantity = tr.getElementsByTagName("td")[3].innerHTML.replace(/<[^>]*>?/gm, "");
            price = tr.getElementsByTagName("td")[5].innerHTML.replace(/<[^>]*>?/gm, "");
          } catch (err) {
            console.warn('ProductModal onSelectChange: unable to read row data', err);
          }
        });
        //consolelog('<>',this.selectedProductId.cgst)
        emit("child-method", {
          id: changableRowKeys.toString(),
          name: name,
          packing: packing,
          quantity: quantity,
          ...
          hsnCode: this.selectedProductId.hsnCode,
          qtyUnit: this.selectedProductId.qtyUnit,
        });
        document.getElementById("form_item_party_name_search").focus();
      }, 400);
    };
  ...
  methods: {
    ...
    test: function (event) {
      console.log("focus", event.keyCode, this.focus);
      switch (event.keyCode) {
        ...
        case 13:
          if (this.table.data.length > 0) {
            console.log("Enter Key", this.selectedProductId);
    ...
    updateSelection() {
      ...
      if (!currentRadioInput) {
        console.warn('updateSelection: no radio input for focus', this.focus);
        return;
      }
      //currentRadioInput.checked=true;
      const temp = currentRadioInput.closest ? currentRadioInput.closest("tr") : null;
      if (!temp) {
        console.warn('updateSelection: no row found for current radio input');
        return;
      }
      ...
      this.selectedProductId.name = temp
        .getElementsByTagName("td")[1]
        .innerHTML.replace(/<[^>]*>?/gm, "");
      this.selectedProductId.packing = temp
        .getElementsByTagName("td")[2]
        .innerHTML.replace(/<[^>]*>?/gm, "");
      this.selectedProductId.quantity = temp
        .getElementsByTagName("td")[3]
        .innerHTML.replace(/<[^>]*>?/gm, "");
      this.selectedProductId.single_unit_price = temp
      ...
      if (Array.isArray(this.table.data) && this.table.data.length > 0) {
        this.table.data.forEach((row) => {
          ...
          if (row.id == selectedRowKey) {
            this.selectedProductId.cgst = Number(row.hsn.cgst >= 0 ? row.hsn.cgst : 0);
            this.selectedProductId.sgst = Number(row.hsn.sgst >= 0 ? row.hsn.sgst : 0);
            this.selectedProductId.lgst = Number(row.hsn.lgst >= 0 ? row.hsn.lgst : 0);
            this.selectedProductId.cess = Number(row.hsn.cess >= 0 ? row.hsn.cess : 0);
            this.selectedProductId.hsnCode = row.hsn.code;
            this.selectedProductId.qtyUnit = row.packing;
            let cgstAmount = 0,
              sgstAmount = 0;

            if (row.hsn && row.sale_rate) {
              let cgstPercentage = Number(row.hsn.cgst) / 100;
              cgstAmount = Number(cgstPercentage) * Number(row.sale_rate);
            }

            if (row.hsn && row.sale_rate) {
              let sgstPercentage = Number(row.hsn.sgst) / 100;
              sgstAmount = Number(sgstPercentage) * Number(row.sale_rate);
            }

            let totalTax =
              Number(sgstAmount) + Number(cgstAmount) + Number(row.sale_rate);

            document.getElementById("mrp").innerHTML = this.formatOfAmount(row.mrp);
            document.getElementById("purchase_rate").innerHTML = this.formatOfAmount(
              row.purchase_rate
            );
            ...
            document.getElementById("rate").innerHTML = this.formatOfAmount(
              row.sale_rate
            );
            ...
            document.getElementById("hsn").innerHTML = row.hsn.code;
            document.getElementById("igst").innerHTML = this.formatOfAmount(row.hsn.lgst);
            document.getElementById("cgst").innerHTML = this.formatOfAmount(
              row.hsn.cgst >= 0 ? row.hsn.cgst : 0
            );
            document.getElementById("sgst").innerHTML = this.formatOfAmount(
              row.hsn.sgst >= 0 ? row.hsn.sgst : 0
            );
    ...
    checkSelectedproduct() {
      if (this.selectedRowKeysValue === undefined) {
        ...
        setTimeout(function () {
          ...
          that.selectedProductId.name = currentRow
            .getElementsByTagName("td")[1]
            .innerHTML.replace(/<[^>]*>?/gm, "");
          that.selectedProductId.packing = currentRow
            .getElementsByTagName("td")[2]
            .innerHTML.replace(/<[^>]*>?/gm, "");
          that.selectedProductId.quantity = currentRow
            .getElementsByTagName("td")[3]
            .innerHTML.replace(/<[^>]*>?/gm, "");
          that.selectedProductId.single_unit_price = currentRow
            .getElementsByTagName("td")[5]
            .innerHTML.replace(/<[^>]*>?/gm, "");
          console.log("Selected Row Key133:", selectedRowKey, this.selectedRowKeysValue);
          if (Array.isArray(that.table.data) && that.table.data.length > 0) {
            that.table.data.forEach((row) => {
              ...
              if (row.id === selectedRowKey || row.id == selectedRowKey) {
                ...
                document.getElementById("purchase_rate").innerHTML = that.formatOfAmount(
                  row.purchase_rate
                );
                document.getElementById("cost").innerHTML = that.formatOfAmount(row.cost);
                document.getElementById("cost_tax").innerHTML = that.formatOfAmount(
                  totalTax
                );
                document.getElementById("purchase_discount").innerHTML = "0.00";
                document.getElementById("rate").innerHTML = that.formatOfAmount(
                  row.sale_rate
                );
                document.getElementById("margin").innerHTML = that.formatOfAmount(
                  row.margin
                );
                document.getElementById("deal_free").innerHTML =
                  (row.free_scheme_1 != null ? row.free_scheme_1 : 0) +
                ...
                document.getElementById("cgst").innerHTML = that.formatOfAmount(
                  row.cgst >= 0 ? row.cgst : 0
                );
                document.getElementById("sgst").innerHTML = that.formatOfAmount(
              ...
              let name = "";
                  myElem.forEach(function (elem, index) {
                    try {
                      ...
                      console.warn('ProductModal checkSelectedproduct: error reading row name', err);
                    }
                  });
