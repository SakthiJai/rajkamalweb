        ...
        <form ref="myForm1" id="addproductform" layout="vertical" @submit.prevent="false">
          <a-row :gutter="16">
                ...
                <a-row :gutter="24">
                  <a-col :xs="24" :sm="24" :md="14" :lg="14">
                    <a-row :gutter="16">
                      ...
                      <a-col :xs="24" :sm="24" :md="12" :lg="12" m>
                        <a-input
                          autocomplete="off"
                          name="product_id"
                          id="product_id"
                        ...
                        <a-row :gutter="16">
                          ...
                          <a-col :xs="24" :sm="24" :md="12" :lg="12">
                            <HscSacSearchInput
                              ...
                              name="hsn_sac"
                              ref="hsn_sac"
                              @valueChanged="(productId) => (formDataLedger.hsn_sac = productId)"
                                @gstValueChanged="(gstValue) => (formDataLedger.gst = gstValue)"
                              @valueSuccess="getStockValue"
                              :productData="data"
                              v-model:value="formDataLedger.hsn_sac"
                              @keyup.enter="focusNext"
                              @keydown="disableSpaceBar"
                            />
                          </a-col>
                        </a-row>
                      </a-col>
            ...
            <a-col :xs="24" :sm="24" :md="8" :lg="8" class="info">
              ...
              <p></p>
              <fieldset >
                 <legend class="basicdetalis">Product Barcode</legend>
                <BarcodeGenerator
                v-if="showBarcode"
                v-model:value=" formDataLedger.barcode_value "
                :height="30"
                :width="2"
                :fontSize="0"
               /></fieldset>
...
export default defineComponent({
  ...
  methods: {
    async getProductData(id) {
      try {
        if (id > 0)  {
          ...
          const response = await axiosAdmin.get(`products/${id}`);
          this.formDataLedger.name = response.data.name;
          console.log("rerrrrrrrr",response.data.name);
          this.formDataLedger.id = response.data.id;
          this.formDataLedger.product_type = response.data.product_type;
          this.formDataLedger.packing = response.data.packing;
          this.formDataLedger.unit_in_decimal = response.data.unit_in_decimal;
          this.formDataLedger.unit_1st = response.data.unit_1st;
          this.formDataLedger.hsn_sac = response.data.hsn_sac;
          console.log(response.data.code);
          this.formDataLedger.gst = response.data.gst;
          this.formDataLedger.company = response.data.company_name;
          console.log(this.formDataLedger.company);
          this.formDataLedger.god_owns = response.data.god_owns;
          this.formDataLedger.opening_stock = response.data.opening_stock;
          this.formDataLedger.low_stock = response.data.low_stock;
          this.formDataLedger.mrp = response.data.mrp;
          this.formDataLedger.purchase_rate = response.data.purchase_rate;
          this.formDataLedger.cost = response.data.cost;
          this.formDataLedger.sale_rate = response.data.sale_rate;
          this.formDataLedger.rate_b = response.data.rate_b;
          this.formDataLedger.free_scheme_1 = response.data.free_scheme_1;
          this.formDataLedger.free_scheme_2 = response.data.free_scheme_2;
          this.formDataLedger.scheme_type = response.data.scheme_type;
          this.formDataLedger.status = response.data.status;
          this.formDataLedger.image = response.data.image_url;
          this.formDataLedger.image_url = response.data.image_url;
          console.log("akash",response.data.image_url);
          this.formDataLedger.color_type = response.data.color_type;
          this.formDataLedger.category_field = response.data.category_field;
          this.formDataLedger.rate_d = response.data.rate_d;
          this.formDataLedger.rate_f = response.data.rate_f;
          this.formDataLedger.item_disc_1_percent = response.data.item_disc_1_percent;
          this.formDataLedger.discount = response.data.discount;
          this.formDataLedger.volume_disc_1 = response.data.volume_disc_1;
          this.formDataLedger.max_disc_percent = response.data.max_disc_percent;
          this.formDataLedger.min_quantity = response.data.min_quantity;
          this.formDataLedger.max_quantity = response.data.max_quantity;
          this.formDataLedger.reorder_days = response.data.reorder_days;
          this.formDataLedger.reorder_qty = response.data.reorder_qty;
          this.formDataLedger.prohibited = response.data.prohibited;
          ...
          this.formDataLedger.as_on_date = response.data.as_on_date;
           this.formDataLedger.barcode_value = response.data.barcode_value || "",
           console.log("ranjith", response.data.barcode_value);



             this.formDataLedger.gst = response.data.gst;
              this.showBarcode = this.formDataLedger.barcode_value;


          console
            .log
            // (this.$refs.tax_category.searchTerm = parseInt( //   response.data.tax_category// ))
            ();

          this.$refs.prodcompsearch.searchTerm = parseInt(response.data.company_id);
          this.$refs.hsn_sac.searchTerm = parseInt(response.data.hsn_sac);
          this.$refs.brandsname.searchTerm = parseInt(response.data.brand_name);
          this.$refs.units.searchTerm = parseInt(response.data.unit_1st);
          this.$refs.godowns.searchTerm = parseInt(response.data.god_owns);
         // this.$refs.category_field.searchTerm = parseInt(response.data.category_field);
         this.$refs.category_field.searchTerm = response.data.category_field
            ? parseInt(response.data.category_field): "";
    ...
    validateField(field) {
      ...
      if (!this.formData || !this.formData[field]) {
    this.rules[field] = 'Loading...';
    return;
  }
  ...
  watch: {
    ...
    // "formDataLedger.tax_category"(newValue) {
    //   this.validateField("tax_category");
    // },
  ...
  mounted() {
    ...
    const that = this;
    setTimeout(function () {
      that.getProductData(that.$route.params.id);
    }, 1500);
  },
  beforeDestroy() {
