<template>
  ...
  >
        ...
        <a-col :xs="24" :sm="24" :md="6" :lg="6">
          <a-form-item name="name">
            <a-select v-model:value="formData.sales_names">
              <a-select-option key="1" value="1" selected> Name </a-select-option>
    ...
    <!--- end-->
    <template #footer class="floats"> </template>
  </a-modal>
...
export default defineComponent({
  ...
  methods: {
    ...
    // customRow(record) {
    //     return {
    //         onClick: (event) => { console.log('record', record, 'event', event); this.closeCustomerListing() }
    //     }
    // },
    ...
    test: function (event) {
      switch (event.keyCode) {
        case 38:
          ...
          console.log(
            "up=>",
            temp.getElementsByTagName("td")[1].innerHTML.replace(/<[^>]*>?/gm, "")
          );
      ...
      if (this.selectedRowKeysValue == undefined) {
        ...
        setTimeout(function () {
          ...
          this.selectedPartyId.name = currentRow
            .getElementsByTagName("td")[2]
            .innerHTML.replace(/<[^>]*>?/gm, "");
          this.selectedPartyId.mobile_number = currentRow
            .getElementsByTagName("td")[1]
            .innerHTML.replace(/<[^>]*>?/gm, "");
          console.log("up=>", this.selectedPartyId);
          ...
          document.querySelectorAll(".ant-radio-input").forEach((elem) => {
            elem.addEventListener("change", function (event) {
              ...
              const modalClose = document.documentElement.querySelector(
                ".ant-modal-close-x"
              );
              if (modalClose) {
                //
