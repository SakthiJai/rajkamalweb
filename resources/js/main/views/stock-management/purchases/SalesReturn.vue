    ...
    <admin-page-table-content>
        <a-card class="page-content-container mt-5 mb-5">
            ...
            <a-col :xs="24" :sm="24" :md="8" :lg="8">
                        <a-form-item :label="$t('Party_name')" name="party_name" ref="input"
                            ...
                            <a-input  v-model:value="formData.party_name" autocomplete="off"
                                ...
                                <a-input hidden v-model="formData.party_id" id="party_id" :placeholder="$t('common.placeholder_default_text', [$t('stock.party_')])"@blur="" />
                                 <a-input hidden v-model:value="formData.party_state" id="party_state" :placeholder="$t('common.placeholder_default_text', [$t('stock.party_')])"@blur="" />
                    ...
                    <InvoiceItemSelectModel v-if="isInvoiceItemSelectModel" :visible="isInvoiceItemSelectModel" :invoice-list="invoiceData.invoiceItems" :url="url"
                        :addEditType="addEditType" :pageTitle="pageTitle" :successMessage="successMessage"
                        @addEditSuccess="handleSuccess" @closed="updateItems" v-on:select-method="updateItems"  v-on:close-method="focusEelment" />
                    <!-- end new modal-->
                ...
                <a-row :gutter="16" class="mt-20 mb-20">
                    ...
                    <a-col :xs="24" :sm="24" :md="4" :lg="4">
                            <a-button type="button" id="saveF8" class="backgrounds"  @click="saveSalesReturnEntry($event)" @keydown="handleSaveKeydown($event)" block>
        ...
        deleteItem()
{
    ...
    handleClosepopup(){
      ...
      getInvoiceDetails(){
        ...
        console.log("this.selectedInvoice",this.formData.selectedInvoice)
        if(this.selectedInvoice!="null")
        {
            this.formData.bill_number=this.formData.selectedInvoice;
            this.spinning = true;
            axiosAdmin//sales/getReturnInvoiceDetails/ sales/getbillInvoiceDetails/
            .get("sales/getReturnInvoiceDetailss/"+this.formData.selectedInvoice)
            .then(response => {  console.log(response)
                // Toastr Notificaiton sgst
                this.returnTypes= response.data.returnTypes;
                this.formData.party_name=response.data.invoiceData.party_name,
                this.formData.party_state=response.data.invoiceData.stock_state,
                ...
                if(response.data.invoiceItems && response.data.invoiceItems.length>0)
                {
                    ...
                    response.data.invoiceItems.forEach((data,index)=> {
                        ...
                        this.formData.items[index].return_qty = data.return_qty;
                        this.formData.items[index].invoice = data.return_invoice;
                        this.formData.items[index].invoice_date = data.invoice_date;
                      //  this.formData.items[index].invoice = data.invoice_number;

                        this.formData.items[index].max_single_unit_price    = data.single_unit_price;
                        this.formData.items[index].unit_id = "";
                        this.formData.items[index].discount_type_id = data.return_reason_code;
                        this.formData.items[index].hsnCode                  = (data.hsnCode);
                         this.formData.items[index].discount_value = data.disc_value;
                        this.formData.items[index].discount_rate = data.disc_value;
                        ...
                        this.formData.items[index].return_reason_code    = data.return_reason_code;
                        console.log("akash")
                       ...
                       //this.formData.items[index].discount_type_id         = (data.discount_type_id);
                        ...
                        document.getElementById('item_product_amount_' + index).value = this.formatNumber(data.subtotal)
                              ...
                              document.getElementById('item_product_packing_' + index).value = data.return_invoice
                        console.log("<.",document.getElementById('item_product_packing_' + index).value,this.formData.items[index].invoice,data.return_invoice );


                       /* document.getElementById('item_product_disc_'+index).value=this.formatNumber(data.disc_value)
                        document.getElementById('item_discount_total_'+index).value=data.discount_rate;*/

                        var lgstTotal = data.product.lgst;
                        console.log("lgstTotal",lgstTotal)
                       ...
                       const cgst_tax_percentage  = Number(this.formData.items[index].cgst);
                                    ...
                                    if(data.return_qty==undefined || data.return_qty=="" || data.return_qty<0){data.return_qty=0}
                                    if(data.single_unit_price==undefined || data.single_unit_price=="" || data.single_unit_price<0){data.single_unit_price=0}
                                    if(discount==undefined || discount=="" || discount<0){discount=0}
                                    const singleItemTotal = Number(data.return_qty*data.single_unit_price);
                                    const taxvalueStr = (lgstTotal) + ",(" + (((lgstTotal) / 100) * singleItemTotal).toFixed(2)+")";
                        document.getElementById("item_product_tax_" + index).innerHTML = taxvalueStr;



                        console.log("checking the value",taxvalueStr)
                        var taxAmount = ((cgst_tax_percentage + sgst_tax_percentage) / 100) * singleItemTotal.toFixed(0);
                        console.log("taxAmount calculatinggggggggg",taxAmount)
                          document.getElementById(`item_product_amount_${index}`).value =(data.subtotal + taxAmount).toFixed(2);
                         document.getElementById("total_goods_value").value =(data.subtotal + taxAmount).toFixed(2);
                         document.getElementById("total_discount_text").innerHTML =((cgst_tax_percentage + sgst_tax_percentage) / 100) * singleItemTotal.toFixed(0);



                         grand_total =  document.getElementById(`item_product_amount_${index}`).value;
                        console.log("totalcgstAmount")
...
console.log("totalcgstAmount")
                    ...
                    var totalcgstAmount = this.getTotalAmount("cgst");
                    console.log("totalcgstAmount",totalcgstAmount)
                    var totalsgstAmount = this.getTotalAmount("sgst");
                    const cessAmount = this.getTotalAmount('cess', null);
                    var a = document.getElementById("igst_amount_2").value ;
                    a = this.formatCurrency(totalcgstAmount+totalsgstAmount);
                    console.log("totalcgstAmounttttttttttttt",a)

                    document.getElementById("igst_amount_4").value = a + cessAmount;
                    ...
                    var b = Number(grand_total)
                    var c = Number(a + cessAmount)
                    console.log("Number(a + cessAmount)",b)
                    console.log("Number(a + cessAmount)",c)
                  //  document.getElementById('grand_total').innerHTML            =   grand_total + (a + cessAmount);
                    ...
                    document.getElementById('igst_amount_0').value              =   this.formatCurrency(totalcgstAmount);
                    document.getElementById('igst_amount_1').value              =   this.formatCurrency(totalsgstAmount);
                    document.getElementById('igst_amount_3').value = this.formatCurrency(cessAmount);


                    let total1 = totalcgst > 0 ? ((totalcgst / 2) / 100) * grand_total : 0;
                    let total2 = totalsgst > 0 ? ((totalsgst / 2) / 100) * grand_total : 0;
                    document.getElementById("igst_amount_2").value = this.formatCurrency(totalcgstAmount + totalsgstAmount);
                    document.getElementById('total_discount_text').innerHTML = this.formatCurrency(totalcgstAmount + totalsgstAmount)
                   // document.getElementById('grand_total').innerHTML = this.formatCurrency((grand_total + totalcgstAmount + totalsgstAmount) - total_disc);
                    document.getElementById('cgst_total_text').innerHTML        =    this.formatCurrency(totalcgstAmount>0?totalcgstAmount:0)
                    document.getElementById('sgst_total_text').innerHTML        =    this.formatCurrency(totalsgstAmount>0?totalsgstAmount:0)



                    document.getElementById('grand_total').innerHTML = this.formatCurrency((grand_total + (cessAmount)));
                   this.formData.total =   (grand_total + (totalcgst+totalsgst+cessAmount));
    ...
    handleClosePayments(input){
       ...
       // setTimeout(function(){document.getElementById('btn-Ledger').focus();},500);
        ...
        {
            console.log("selectedKeys=>",selectedKeys,this.invoiceData.invoiceItems);
                ...
                let itemIndex = 0;
                    this.invoiceData.invoiceItems.forEach((data,index)=>{
                        ...
                        if (list.indexOf(data.product_id) != -1 && this.formData.items[index].selected == false) {
                            data.subtotal = 0;
                            this.formData.items[itemIndex].discount_type_id = data.discount_type_id;
                            this.formData.items[itemIndex].item_id = data.product_id;
                            this.formData.items[itemIndex].invoice = data.invoice_number;
                            this.formData.items[itemIndex].invoice_date = data.order_date;
                            this.formData.items[itemIndex].item_name = data.product_name;

                            this.formData.items[itemIndex].item_id = data.product_id
                                ; document.getElementById('item_product_name_' + itemIndex).value = data.product_name
                        this.formData.items[itemIndex].unit_id                  = "";
                        this.formData.items[itemIndex].quantity                 = this.formatNumber(data.quantity);
                        document.getElementById('item_product_quantity_'+itemIndex).value=data.quantity
                        this.formData.items[itemIndex].mrp                      = this.formatNumber(data.mrp);
                        this.formData.items[itemIndex].single_unit_price        = this.formatNumber(data.single_unit_price);
                        document.getElementById('item_product_price_'+itemIndex).value=this.formatNumber(data.single_unit_price)

                        this.formData.items[itemIndex].discount_rate            = "";
                        this.formData.items[itemIndex].amount                   = "";
                        this.formData.items[itemIndex].maxquantity              = this.formatNumber(data.stock);
                    this.formData.items[itemIndex].max_single_unit_price    = this.formatNumber(data.discount);
                        this.formData.items[itemIndex].packing                  = data.pack;
                        this.formData.items[itemIndex].cgst                     = (Number(data.product.cgst)>=0?data.product.cgst:0);
                        console.log(this.formData.items[itemIndex].cgst,'cgst=>',data.cgst);
                        this.formData.items[itemIndex].sgst                     = (Number(data.product.sgst)>=0?data.product.sgst:0);
                        this.formData.items[itemIndex].selected                 = true;
                        this.formData.items[itemIndex].cess                     = (Number(data.product.cess)>=0?data.product.cess:0);
                        ...
                        document.getElementById('item_product_amount_'+itemIndex).value=this.formatNumber(data.subtotal)
                    ...
                    setTimeout(function()
                    {
                        const totalGstAmount = (totalcgst>0?((totalcgst/2)/100)*grand_total:0) + (totalsgst>0?((totalsgst/2)/100)*grand_total:0)
                        ...
                        document.getElementById('igst_amount_0').value              =   temp.formatCurrency(totalcgst>0?((totalcgst/2)/100)*grand_total:0);
                        document.getElementById('igst_amount_1').value              =   temp.formatCurrency(totalsgst>0?((totalsgst/2)/100)*grand_total:0);
                        temp.spinning= false;
                    ...
                    console.log(this.formData.items) //item_product_return_qty_0
                    document.getElementById('item_product_return_qty_0').focus()
   ...
   handleClose() {
        ...
        getQuantity(index,event) {
            ...
            let  quantity              = document.getElementById("item_product_return_qty_"+index).value;
          ...
          //  let priceToUse = singlePrice > 0 ? singlePrice : maxPrice;
            ...
            document.getElementById("item_product_amount_"+index).value =this.formatCurrency(singleItemTotal+totalDisc);
         ...
         this.formData.tax_amount                             =  totalSgstAmount+ totalCgstAmount;
            ...
            document.getElementById("grand_total").innerHTML     = totalAmount>0? this.formatCurrency((totalAmount+cessAmount)):0;
        ...
        if (returnQty > quantity) {
            ...
            event.target.value = quantity; // Sync the input field
            this.$nextTick(() => {
                document.getElementById(`item_product_return_qty_${index}`).focus();
            });
        }
        ...
        saveSalesReturnEntry(event)
        {
            ...
            this.spinning= true;
            axiosAdmin
            .post("sales/salesReturn", this.formData)
            .then(response => {
                console.log(response)
                // Toastr Notificaiton
                this.spinning= false;
    ...
    if (event.keyCode === 13) { // Enter key
        ...
        resetFormData()
        {
            ...
            this.formData=({
                ...
                selectedInvoice:localStorage.getItem("selectedInvoice"),
                items:[
                    {index:1,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:2,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:3,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:4,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:5,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:6,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:7,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:8,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:9,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                    {index:10,item_id:null,item_name:null,unit_id:null,quantity:'',mrp:null,single_unit_price:null,discount_type_id:0,discount_value:0,discount_rate:null,amount:null,maxquantity:0,max_single_unit_price:"",packing:null,invoice:null,invoice_date:null,return_qty:null,return_reason_code:null,selected:false},
                ],
