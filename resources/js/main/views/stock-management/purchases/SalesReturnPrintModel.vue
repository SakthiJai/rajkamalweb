    ...
    >
      <option value="1">General Billing</option>
        ...
        <template #footer>
            ...
            <button  @click="previewpdf" type="button"  title="Ledger" class="btn default-btn ng-star-inserted ">
                <span class="box"><span class="shortcut ng-star-inserted" ><code>F</code></span ><span class="ng-star-inserted">Preview</span>
                </span>
                <span class="effect"></span>
            </button>
            <button
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            focusedButton: "print", // Added for button focus tracking
        };
    },

    mounted() {
        document.getElementById("salesreturnprint").addEventListener('keydown', this.handleKeyDownPrint);
        this.autoFocusInput();
    },

    beforeDestroy() {
        document.getElementById("salesreturnprint").removeEventListener('keydown', this.handleKeyDownPrint);
    },
    ...
    methods: {
        ...
        handleKeyDownPrint(event) {
            ...
            if (event.key === "Enter") {
                ...
                if (!this.isModalVisible) {
                    ...
                    // If modal is open, trigger action based on focused button
                    if (this.focusedButton === "print") {
                        this.printpdf();
                    } else if (this.focusedButton === "preview") {
                        this.previewpdf();
                    } else if (this.focusedButton === "whatsapp") {
                        this.WhatsAppOutlined();
                    }
                }
            }
        ...
        previewpdf() {
            ...
            this.loading = true;
            axiosAdmin
                .post("sales-return/getSalesReturnPdf", { invoice: document.getElementById('form_item_bill_number').value })
                .then(response => {
                    let w = window.open(response, '_blank');
                    w.print();
                })
