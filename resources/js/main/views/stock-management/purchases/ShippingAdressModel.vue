<template>
      ...
      <template v-slot:title>
        <div style="display: flex; justify-content: space-between; align-items: center">
          ...
          <div @keydown="onKeydown">
            ...
            <!--  customer modal-->
            <ShippingCreateModal
              v-if="isShippingCreateModal"
              :visible="isShippingCreateModal"
            ...
            <div class="customer table-responsive">
              ...
              >
                <template #bodyCell="{ column, record, rowIndex }">
                  ...
                  <template v-if="column.dataIndex === 'shipping_pincode'">
                    <a-typography-text
                      v-if="record.adjustment_type === 'add'"
                      type="success"
                      strong
                  ...
                  <template v-if="column.dataIndex === 'shipping_city'">
                    <a-typography-text
                      v-if="record.adjustment_type === 'add'"
                      type="success"
                      strong
                  ...
                  <template v-if="column.dataIndex === 'shipping_contact_number'">
                    <a-typography-text
                      v-if="record.adjustment_type === 'add'"
                      type="success"
                      strong
                    >
                      +{{ record.shipping_contact_number }}
                    </a-typography-text>
                  </template>
                  <template v-if="column.dataIndex === 'action'">
                    <a-button @click="editReturn(record.id)">
                      <template #icon>
                        <EditOutlined />
                      </template>
                    </a-button>
  ...
  <!-- customer edit-->
  <ShippingEdit
    v-if="isShippingeditPopup"
    :visible="isShippingeditPopup"
    :formDataLedger="formDataLedger"
    :url="url"
    :addEditType="addEditType"
    :pageTitle="pageTitle"
...
import fields from "./SalesShipDetails/fields";
import crud from "../../../../common/composable/crud";
import common from "../../../../common/composable/common";
import ShippingCreateModal from "./ShippingCreateModal.vue";
import PopupModal from "./PopupModal.vue";
import { Modal } from "ant-design-vue";
import { createVNode } from "vue";
import ShippingEdit from "./ShippingEdit.vue";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const isShippingeditPopup = ref(false);
    ...
    const showDeleteConfirm = (id) => {
      ...
      Modal.confirm({
        ...
        content: t("Are you sure you want to delete this Shipping?"),
        autoFocusButton: "ok",
        centered: true,
        okText: t("common.yes"),
        okType: "danger",
        ...
        onOk() {
          axiosAdmin.delete(`${props.orderType}/${id}`).then(() => {
            ...
            notification.success({
              ...
              description: t("Shipping Address Deleted Successfully"),
            });
          });
        },
        onCancel() {},
    ...
    const reFetchDatatable = (formParty) => {
      ...
      crudVariables.tableUrl.value = {
        url:
          `shipping-list?fields=id,xid,ledger_id,shipping_address,shipping_city,shipping_contact_number,is_temporary,shipping_pincode&party=` +
          formParty,
        filterableColumns,
      };
  ...
  data() {
    return {
      ...
      isLoading: false,
      isShippingCreateModal: false,
      isShippingeditPopup: false,
      dialog: false,
      formData: {
        sales_names: "1",
      },
  ...
  mounted() {
    // Listen for keydown events on the 'ShippingListvue' element
    document
      .getElementById("ShippingListvue")
  ...
  methods: {
    editReturn(id) {
      ...
      console.log(id);
      try {
        this.isShippingeditPopup = true; // Set the popup to true to show it
      } catch (error) {
    ...
    showShippingCreateModal() {
      console.log("customer", this.selectedPartyId);
      this.isShippingCreateModal = true;
    },
    ...
    handlePopup() {
      ...
      this.isShippingeditPopup = false;
    },
    ...
    onClose() {
      this.isShipDetailsVisible = false;
      this.$emit("closed");
    },
      ...
      switch (event.keyCode) {
        case 38:
          ...
          if (this.table.data.length > 0) {
            ...
            this.selectedPartyId.shipping_address = temp
              .getElementsByTagName("td")[3]
              .innerHTML.replace(/<[^>]*>?/gm, "");
            this.$emit("shipping-method", this.selectedPartyId);
            if (this.focus === null) {
              this.focus = 0;
        ...
        case 40:
          if (this.table.data.length > 0) {
            ...
            this.selectedPartyId.shipping_address = temp1
              .getElementsByTagName("td")[3]
              .innerHTML.replace(/<[^>]*>?/gm, "");
          ...
          document.querySelectorAll(".ant-radio-input").forEach((elem) => {
            elem.addEventListener("change", function (event) {
              ...
              if (modalCloseButton) {
                ...
                that.$emit("shipping-method", that.selectedPartyId);
    ...
    handleCutomerCloses() {
      console.log("Closed");
      this.isShippingCreateModal = false;
      this.isShippingeditPopup = false;
      this.autoFocusInput();
      this.reFetchDatatable(this.formParty);
    },
