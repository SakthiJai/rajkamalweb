    ...
    <a-row :gutter="[16, 16]">
      <a-col :xs="24" :sm="24" :md="12" :lg="10" :xl="10">
        <a-space>
          <template
            v-if="
              permsArray.includes(`${userType}_create`) || permsArray.includes('admin')
            "
          >
            <a-space>
          ...
          <a-button
            v-if="
              ...
              (permsArray.includes(`${userType}_delete`) || permsArray.includes('admin'))
            "
            type="primary"
    ...
    <a-row v-if="userType && userType == 'users'">
      <a-col :span="24">
        <a-tabs v-model:activeKey="searchStatus" @change="setUrlData">
          <a-tab-pane key="all" :tab="`${$t('common.all')} ${$t('menu.users')}`" />
          <a-tab-pane
            v-for="usrStatus in userStatus"
            :key="usrStatus.key"
          ...
          <a-table
            :row-selection="{
              ...
              getCheckboxProps: (record) => ({
                disabled:
                  user.xid == record.xid || record.is_walkin_customer ? true : false,
                name: record.xid,
              }),
            }"
          ...
          >
            <template #bodyCell="{ column, text, record }">
              <template v-if="column.dataIndex === 'name'">
                <a-typography-link type="primary" @click="viewItem(record)">
            ...
            <template v-if="userType && userType != 'users'" #summary>
              <a-table-summary-row>
                ...
                <a-table-summary-cell :col-span="1">
                  <a-typography-text strong>
                    <a-tooltip v-if="totals.totalOpeningBalance < 0">
                      <ArrowUpOutlined :style="{ color: 'red' }" />
                      {{ formatAmountCurrency(totals.totalOpeningBalance) }}
                    </a-tooltip>
                    <a-tooltip v-else>
                      <span v-if="totals.totalOpeningBalance > 0">
                        <ArrowDownOutlined
                          :style="{
                            color: 'green',
                          }"
